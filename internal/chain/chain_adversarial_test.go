...
func TestAdversarial_InvalidSignatureRejection(t *testing.T) {
	...
	tx := chain.Tx{
		...
		Fee:       10,
		Nonce:     0,
		PublicKey: "0x" + adversarialToHex(pub),
	}
...
// newTestGenesis creates a test genesis state.
...
// adversarialToHex encodes bytes to a hex string.
func adversarialToHex(b []byte) string {
