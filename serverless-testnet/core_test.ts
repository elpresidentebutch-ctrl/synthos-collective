...
Deno.test("six validators finalize with five-of-six quorum despite out-of-order votes", async () => {
  const ids = ["cloudflare-1", "deno-1", "deno-2", "deno-3", "deno-4", "deno-5"];
  const pairs = ids.map(() => nacl.sign.keyPair());
  const genesis: Genesis = {
    protocolVersion: 1, chainId: "synthos-testnet-test", network: "synthos-testnet-test",
    genesisTime: new Date(0).toISOString(), heartbeatMs: 15000, quorum: 5,
    validators: ids.map((id, index) => ({ id, publicKey: b64(pairs[index].publicKey), url: `https://${id}.test` })), allocations: {},
  };
  const nodes = new Map<string, ReliableValidatorNode>();
  let now = 30_000;
  const fetcher: typeof fetch = async (input, init) => {
    const request = new Request(input, init);
    const node = nodes.get(new URL(request.url).hostname.replace(".test", ""));
    return node ? await node.handle(request, { waitUntil: () => undefined }) : new Response("missing", { status: 404 });
  };

  ids.forEach((id, index) => {
    const store = new MemoryStore();
    const inner = new ValidatorNode({ validatorId: id, privateKey: b64(pairs[index].secretKey.slice(0, 32)), genesis, store,
      fetcher, now: () => now, logger: { log() {}, warn() {}, error() {} } });
    nodes.set(id, new ReliableValidatorNode(inner, store));
  });
