...
class DenoKvStore implements Store {
  ...
  private key(value: string): Deno.KvKey { return value.split("/").filter(Boolean); }
  async get<T>(key: string): Promise<T | null> { return (await this.kv.get<T>(this.key(key), { consistency: "strong" })).value; }
  async set<T>(key: string, value: T): Promise<void> { await this.kv.set(this.key(key), value); }
...
if (!validatorId || !privateKey || !genesisJson) throw new Error("VALIDATOR_ID, VALIDATOR_PRIVATE_KEY, and GENESIS_JSON are required");

const kv = await Deno.openKv();
const store = new DenoKvStore(kv);
const node = new ReliableValidatorNode(new ValidatorNode({
  validatorId,
  privateKey,
...
Deno.cron("synthos-validator-safety-tick", "* * * * *", async () => {
  try { await node.tick(); }
  catch (error) { console.error(JSON.stringify({ event: "safety_tick_error", validatorId, error: String(error) })); }
});

Deno.serve((request) => {
  const context: RuntimeContext = { waitUntil(promise) {
