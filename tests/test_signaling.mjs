...
async function runTests() {
  ...
  if (!REGISTRY_URL) {
    ...
    console.log("Usage: node tests/test_signaling.mjs https://your-registry.workers.dev\n");
  ...
  try {
    const { status, data } = await fetchJson(`${REGISTRY_URL}/register`, {
      ...
      body: JSON.stringify({ name: testName, url: "https://test-peer.example.com", cloud: "test" }),
    });
  ...
  try {
    ...
    assert(found?.url === "https://test-peer.example.com", "test peer URL correct");
